...
export class EmailService {
  ...
  async sendMessage(email, link, messageContent) {
    ...
    const uploadResult = await this.uploadService.uploadByImageData(
      path,
      qrImageData.split(',')[1],
    );
    ...
    const content = await EmailBuilder.messageEmail(
      link,
      messageContent,
      qrImagePath,
    );
  ...
  async sendRegisterMail(email, body) {
    const content = await EmailBuilder.registerMail(body);
    return this.sendEmail(email, 'New Registration', content);
  }
}
