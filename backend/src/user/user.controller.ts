...
export class UserController {
  ...
  async change_avatar(
    ...
    const user = await this.userService.updateUserAvatar(req.user.id, body.path);
  ...
  @Get('all_useravatars')
  @ApiOkResponse({ type: UserAssetDto })
  async getAllUserAvatars(): Promise<any> {
    const icons = await this.userService.getAllUserAvatars();
    return {
      success: true,
      useravatars: icons.map((icon) => icon.toCollectionDto()),
    };
  }

  @Get('all_userbackgrounds')
  @ApiOkResponse({ type: UserAssetDto })
  async getAllUserBackgrounds(): Promise<any> {
    const icons = await this.userService.getAllUserBackgrounds();
    return {
      success: true,
      userbackgrounds: icons.map((icon) => icon.toCollectionDto()),
    };
  }
}
